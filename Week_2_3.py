# -*- coding: utf-8 -*-
"""
Created on Sat Jun  6 13:24:53 2020

@author: Administrator
"""

#Assuming that the planet has no luminosity of it's own and only reflects the light 
#of the star.
...
lum_1_transit = np.linspace(1.0 + 1.0e-4/2, 1.0, 500)

#luminosity after egress
x5 = np.linspace((R + rP), 10.0*R, 500)
lum_5_transit = np.linspace(1.0, 1.0 + 1.0e-4/2, 500)
...
#luminosity before ingress
lum_5_occultation = np.linspace(1.0 + 1.0e-4, 1.0 + 1.0e-4/2, 500)

#luminosity during ingress
def L_in(x4, R, rP):
    first = (1.0e-4/np.pi)*np.arccos((R - x4)/rP)
    second = (1.0e-4/(np.pi*rP))*(R - x4)*np.sqrt(1.0 - (R - x4)**2/rP**2)
...
#luminosity during egress
def L_out(x2, R, rP):
    first = (1.0e-4/np.pi)*np.arccos((x2 - R)/rP)
    second = (1.0e-4/(np.pi*rP))*(x2 -  R)*np.sqrt(1.0 - (x2 - R)**2/rP**2)
    return (1.0 - first + second + 1.0e-4)
lum_out = L_out(-x2, R, rP)

#luminosity after egress
lum_1_occultation = np.linspace(1.0 + 1.0e-4/2, 1.0 + 1.0e-4, 500)
...
plt.title("Full Light Curve for a Circular Orbit")
plt.savefig("Week_2_3.png")
plt.show()
